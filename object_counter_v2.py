...
from object_detection.trackAndDetect.centroidtracker import CentroidTracker
from object_detection.trackAndDetect.trackableobject import TrackableObject
...
ap.add_argument("-f", "--fline", type=float, default=0.6,
                help="region in the video frame after which we count a vehicle")
...
ct = CentroidTracker(maxDisappeared=20, maxDistance=10)
trackers = []
trackableObjects = {}
...
while True:
    ...
    if totalFrames % args["skip_frames"] == 0:
        ...
        for i in np.arange(0, detections.shape[2]):
            ...
            if confidence > args["confidence"]:
                ...
                print(f'Bottom left corner: {rect.bl_corner()}')
                print(f'Top right corner: {rect.tr_corner()}')
    ...
    # moving 'up' or 'down'
    cv2.line(frame, (0, int(H // args["fline"])), (W, int(H // args["fline"])), (0, 255, 255), 2)
...
print("[INFO] Total Vehicles Going Down: {:}".format(totalDown))
print("[INFO] Total Vehicles Going Down: {:}".format(totalUp))

with open(args["log"], 'w') as f:
    print("[INFO] elapsed time: {:.2f}".format(fps.elapsed()), file=f)
    print("[INFO] approx. FPS: {:.2f}".format(fps.fps()), file=f)
    print("[INFO] Total Vehicles Going Down: {:}".format(totalDown), file=f)
    print("[INFO] Total Vehicles Going Up: {:}".format(totalUp), file=f)
